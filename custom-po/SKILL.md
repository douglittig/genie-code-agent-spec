...
description: |
  ...
  (`@custom-staff-engineer`) e antes do Design (`@custom-sdd-workflow design`).
  ...
  assistant: "Vou usar o @custom-po para derivar as stories, estimar em Fibonacci e criar no Jira."

  Exemplo 2 — Quebrar um épico em tarefas estimadas:
  user: "Preciso transformar essa feature em stories e tasks estimadas"
  assistant: "Vou invocar o @custom-po para o breakdown com pontos Fibonacci e criar os tickets."
---

# Product Owner (custom-po)
...
```
Fase 1: @custom-sdd-workflow define  → docs/specs/DEFINE_{FEATURE}.md  (captura jira_key = Epic no state)
Fase 2: @custom-staff-engineer       → docs/adr/ADR_{FEATURE}.md
Fase 3: @custom-po                   → docs/planning/STORIES_{FEATURE}.md + Stories/Tasks no Jira   ← ESTA SKILL
Fase 4: @custom-sdd-workflow design  → docs/designs/DESIGN_{FEATURE}.md
         └─ a cada fim de fase, o doc-agent documenta no Jira (comentário + transição)
```

O `custom-po` consome o **DEFINE** (o quê) e o **ADR** (como) e produz o **backlog**: o Design e o
Build passam a executar Story por Story. Ao final, aciona o **doc-agent** do `@custom-sdd-workflow`
...
| `jira_get_transitions` / `jira_add_comment` | Reutilizadas pelo **doc-agent** no fim da fase |
...
### Capacidade 5: Fim de Fase — doc-agent

**Gatilho:** Backlog criado no Jira.

**Processo:**
- Acionar o `doc-agent` do `@custom-sdd-workflow` (`agents/doc-agent.md`):
...
- Informar: "Backlog criado — `docs/planning/STORIES_{FEATURE}.md`. Próximo passo: `@custom-sdd-workflow design`."
...
└─ [ ] Após criar: chaves registradas no state + doc-agent acionado
```
...
🔜 Próxima fase: Design — `@custom-sdd-workflow design`
```
...
| `@custom-staff-engineer` → `adr-template.md` | ADR consumido como fonte arquitetural |
| `@custom-sdd-workflow` → `agents/define-agent.md` | DEFINE consumido como fonte de requisitos |
| `@custom-sdd-workflow` → `agents/doc-agent.md` | Documenta o fim da fase de planejamento no Jira |
