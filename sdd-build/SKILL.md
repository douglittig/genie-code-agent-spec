...
name: sdd-build
description: |
  Executor de implementação com delegação às skills Databricks (Fase 5 do workflow SDD — @sdd-workflow).
  ...
  assistant: "Vou usar o sdd-build para executar a implementação."

  Exemplo 2 — Usuário quer implementar uma feature projetada:
  user: "Implemente o sistema de autenticação de usuário"
  assistant: "Deixa eu invocar o sdd-build para construir a partir do design."
---
...
> **Template desta skill:** [`BUILD_REPORT_TEMPLATE.md`](BUILD_REPORT_TEMPLATE.md) — estrutura do `.claude/sdd/reports/BUILD_REPORT_{FEATURE}.md`
...
**Fallback sem ferramentas:** se `ruff`/`mypy`/`pytest` não existirem no ambiente, usar a stdlib —
`python3 -m py_compile {arquivo}` (sintaxe) e `python3 -m unittest` (testes) — e registrar no
BUILD_REPORT que o lint estilístico/type check não foi coberto. **Nunca** pular a verificação por
falta de ferramenta.
...
`concluída`, caminho do report, resultado da verificação) e **acionar a skill `@sdd-doc`**:
...
Depois, sugerir a próxima fase: Ship (`@sdd-ship`).
