...
description: |
  ...
  assistant: "Vou usar o sdd-doc para postar o resumo no ticket e mover para Em andamento."

  Exemplo 2 — Build verificado, pronto para revisão:
  user: "Build passou nos testes, atualize o Jira"
  assistant: "Deixa eu invocar o sdd-doc para comentar o BUILD_REPORT e transicionar para Em revisão."
---
...
O `sdd-doc` **não é uma fase** — é um *hook transversal* chamado ao **final de cada fase**:

```
Define ─┐
ADR ────┤
PO ─────┤
Design ─┼──► sdd-doc ──► Jira (comentário + transição) ──► .claude/sdd/state/{FEATURE}.md
Build ──┤
Ship ───┘
```

Cada fase termina com o **Protocolo de Fim-de-Fase** (ver `@sdd-workflow`): gerar artefato →
atualizar state → **chamar o sdd-doc** → sugerir próxima fase.

---

## Arquitetura de Conhecimento

**ESTA SKILL SEGUE RESOLUÇÃO SKILLS-FIRST. O state é a fonte da verdade; o Jira é o destino.**
...
> Limite do Genie Code: 20 ferramentas por workspace. Esta skill usa **4** do lado Jira.
...
**Disciplina (espelha o Confluence intake do sdd-define):** operar **somente** na `jira_key`
...
3. Montar o comentário com `JIRA_UPDATE_TEMPLATE.md` (resumo, artefato, origem, gate, próxima fase)
...
6. Se as ferramentas MCP Jira **não estiverem disponíveis** (ou a chamada falhar) → tratar como
   **modo pendente**: registrar `pendente` no state, avisar o usuário e seguir o fluxo — nunca
   bloquear a fase por indisponibilidade do Jira
...
```markdown
## sdd-doc — Fase {NOME} (Jira pendente)
