...
**Contexto:** O arquivo `sdd-dev-workflow/pr-template.md` é um placeholder genérico.
...
**Origem:** Sessão de criação da skill `sdd-dev-workflow`.

---

### 2. PR Review Rules Corporativas
**Contexto:** O Passo 7 do `sdd-dev-workflow` tem um placeholder "a ser lapidado com regras da empresa".
...
**O que falta:** Criar um comando ou instrução dentro da skill `sdd-dev-workflow` (ou skill dedicada) que guie o Genie Code no processo de staging + commit com mensagem estruturada.
**Origem:** Sessão de criação da skill `sdd-dev-workflow`.

---

### 4. Branch Protection Rules no Repositório Centralizado
**Contexto:** A regra de "nunca commitar na main" está documentada em AGENTS.md/CLAUDE.md e na skill `sdd-dev-workflow`, mas é apenas instrução — sem enforcement técnico.
...
**Origem:** Pesquisa de viabilidade MCP para sdd-dev-workflow — 2026-04-28.
...
**Origem:** Leitura do paper Day 1 durante a evolução do `sdd-workflow`.
...
| Item | Sessão |
|------|--------|
| Skill `sdd-dev-workflow` criada | 2026-04-28 |
